...
RedisModuleString* RAI_GetBackendsPath(RedisModuleCtx *ctx) {
  ...
  if (RAI_BackendsPath != NULL) {
    ...
    backends_path = RedisModule_CreateStringPrintf(ctx, "%s/backends", RedisModule_StringPtrLen(module_path, NULL));
  }
...
int RAI_LoadBackend(RedisModuleCtx *ctx, int backend, const char* path) {
  ...
  if (path[0] == '/') {
    ...
    fullpath = RedisModule_CreateStringPrintf(ctx, "%s/%s", RedisModule_StringPtrLen(backends_path, NULL), path);
  }
